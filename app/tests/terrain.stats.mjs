...
// Hügel: p95 der Auslenkung ≈ hillAmp (±10 %)
{
    const amp = PARAM_FIELDS.hillAmp({ hillAmp: 17 });
    const d = sample((x, y, s) => Math.abs(fbm(x, y, s + 5.1, 5) * amp), 30);
    const p95 = quantile(d, 0.95);
    check(Math.abs(p95 - 17) <= 1.7, `Hügel p95 ${p95.toFixed(1)} m ≈ hillAmp 17 m`);
}
...
function maxHeight(p) {
    ...
    for (let seed = 1; seed <= 20; seed++) {
        ...
        for (let j = 0; j < N; j++) for (let i = 0; i < N; i++) {
            ...
            let h = u.baseLevel + fbm(wx * u.hillScale, wy * u.hillScale, sd, 5) * u.hillAmp;
            ...
            h += rimF * (0.6 + 0.4 * fbm(wx * u.rimScale, wy * u.rimScale, sd + 505.3, 3)) * u.rimAmp;
            if (h > mx) mx = h;
        }
    }
    return mx;
...
const base = {
    mapSize: 400, baseLevel: 30, hillAmp: 8, hillWave: 120, mountainAmp: 60, mountainWave: 180, clusterWave: 220,
...
const extreme = { ...base, baseLevel: 60, hillAmp: 30, mountainAmp: 150, mountainCoverage: 100, cliffDrop: 60, cliffCoverage: 100, rimAmp: 100, rimZone: 150 };
