...
import { EROSION_RES, EROSION_FLOATS, encodeErosion } from './uniforms.js';

const STEPS = ['fluxStep', 'water', 'erode', 'carry', 'thermal']; // Reihenfolge je Iteration (→ erosion.wgsl)

export function createErosion(device) {
    const cells = EROSION_RES * EROSION_RES, groups = EROSION_RES / 16;
    ...
    // raw: GPU-Puffer mit EROSION_RES² Roh-Höhen in m ab Byte 0; p = params + mapSize.
    ...
    function run(raw, p, erode) {
        ...
        enc.copyBufferToBuffer(raw, 0, bufs.b, 0, cells * 4);
        enc.copyBufferToBuffer(raw, 0, bufs.delta, 0, cells * 4);
        for (const k of ['bTmp', 'state', 'sedTmp', 'flux', 'flow']) enc.clearBuffer(bufs[k]);
